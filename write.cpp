...
bool has_args(int argc, char *argv[], string target) {
...
int main(int argc,
         char *argv[]) { // run $ dd if=/dev/zero of=res.img bs=8192 count=8192
  ...
  if (has_args(argc, argv, "--generate-header")) {
    ofstream output("res.img.inc", ios::out);
    output << ";; generated by writefat\n";
    output << "fat_loc: dw " << dec << bootsec.BPB_RsvdSecCnt << endl;
    output << "data_loc: dw " << dec
           << bootsec.BPB_RsvdSecCnt + bootsec.BPB_NumFATs * bootsec.BPB_FATSz32
           << endl;
    output.close();
  } else {
